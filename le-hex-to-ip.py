...
def ipconvert(hexstr):
    if len(hexstr) == 8:
        ...
        return 'WRONG HEXSTRING LENGTH'


def main():
    parser = argparse.ArgumentParser(usage='usage: ' + os.path.basename(__file__) + ' hexstring\n' + __description__)
    parser.add_argument('hexstring', metavar='HEXSTRING', nargs='*', default=sys.stdin,
                        help='hexstrings to convert, if none, take from stdin')
    ...
    for hexstr in args.hexstring:
        try:
            if hexstr.index(':'):
                ...
                print(str(hexstr) + ':' + str(int(strings[1], 16)))
        except ValueError:
